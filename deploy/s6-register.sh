#!/usr/bin/env bash
# ── Registro del servicio s6 de eva-dutch-bot ───────────────────────────────
...
#
# Uso:  bash /opt/data/eva-dutch-bot/deploy/s6-register.sh
set -euo pipefail

APP_DIR="${1:-/opt/data/eva-dutch-bot}"
SVC=/run/service/eva-dutch-bot
...
# shellcheck shell=sh
# eva-dutch-bot — Lingua (bot de Telegram, polling propio, puerto 3023).
set -e
export HOME=/opt/data
cd /opt/data/eva-dutch-bot
...
#!/command/with-contenv sh
# shellcheck shell=sh
log_dir="/opt/data/eva-dutch-bot/logs"
if [ "$(id -u)" = 0 ]; then
...
echo "   curl http://127.0.0.1:3023/health"
